...
class speaker {
 public:
  speaker(uint32_t uid, int sample_rates, file_format format, int slice,
 ...
 private:
  uint32_t uid_;
  audio_recorder *recorder_;

  std::deque<char> data_;
};

speaker::speaker(uint32_t uid, int sample_rates, file_format format,
    int slice, const char *file_prefix) :uid_(uid) {
  recorder_ = NULL;
  if (file_prefix != NULL) {
    recorder_ = audio_recorder::create(file_prefix, 1, sample_rates,
        format, slice);
    if (!recorder_->initialize()) {
      SAFE_LOG(ERROR) << "Failed to open the recording file: " << file_prefix;
      delete recorder_;
      recorder_ = NULL;
      return;
    }
...
bool speaker::get_pcm_data(void *data, int bytes) {
  ...
  if (recorder_ != NULL) {
    recorder_->write_audio(data, bytes);
  }
...
audio_event_handler::audio_event_handler(int read_fd, int write_fd, uint32_t uid,
    ...
    int32_t samples, int idle, bool batch, const char *path_prefix, bool pstn, int slice, const char *codec)
:read_fd_(read_fd), write_fd_(write_fd), uid_(uid), vendor_key_(vendor_key),
    ...
    sample_rates_(samples), channel_idle_(idle), is_pstn_(pstn), slice_(slice),
    stored_fmt_(!strcmp(codec, "pcm") ? kRawPCM : kMp3) {
  applite_ = NULL;
  joined_ = false;
  up_pcm_recorder_ = NULL;
  down_pcm_recorder_ = NULL;
  reader_ = NULL;
  writer_ = NULL;
...
int audio_event_handler::run() {
  ...
  char buf[512];
  snprintf(buf, 511, "{\"audioEngine\":{\"audioSampleRate\":%d"
      ", \"useAudioExternalDevice\":true}}", sample_rates_);

  applite_->setProfile(buf, true);
...
cleanup:
  std::cout << "Max users during the call: " << max_users_ << std::endl;
  close(timer_fd);
  return ret_code;
}
...
void audio_event_handler::on_voice_data(const string &data) {
  if (joined_ && data.size() > 0) {
    ...
    if (up_pcm_recorder_)
      up_pcm_recorder_->write_audio(&data[0], data.size());
  }
}
...
void audio_event_handler::onJoinChannelSuccess(const char *channel, uid_t uid,
  ...
  // open the upstream/down pcm recorder to store pcm data.
  if (!joined_ && !is_batch_mode_ && g_record_pcm) {
    int64_t ts = now_us();
    char location[512];
    snprintf(location, 511, "%s/%" PRId64 "_%s_%s_up",
        g_record_directory.c_str(), ts, channel_name_.c_str(),
        vendor_key_.c_str());

    up_pcm_recorder_ = new (std::nothrow)pcm_recorder(location);
    if (!up_pcm_recorder_ || !up_pcm_recorder_->initialize()) {
      SAFE_LOG(ERROR) << "Failed to open the recording file: " << location;
      delete up_pcm_recorder_;
      up_pcm_recorder_ = NULL;
    }

    snprintf(location, 511, "%s/%" PRId64 "_%s_%s_down",
        g_record_directory.c_str(), ts, channel_name_.c_str(),
        vendor_key_.c_str());

    down_pcm_recorder_ = new (std::nothrow)pcm_recorder(location);
    if (down_pcm_recorder_ == NULL || !down_pcm_recorder_->initialize()) {
      SAFE_LOG(ERROR) << "Failed to open the recording file: " << location;
      delete down_pcm_recorder_;
      down_pcm_recorder_ = NULL;
    }
...
bool audio_event_handler::onPlaybackFrame(void *audioSample, int nSamples,
  ...
  if (down_pcm_recorder_) {
    down_pcm_recorder_->write_audio(audioSample, bytes);
  }
  ...
  if (it == speakers_.end()) {
    ...
    char location[512];
    if (is_batch_mode_) {
      snprintf(location, 511, "%s%s-0", path_prefix_.c_str(),
          channel_name_.c_str());
      stored = location;
    }

    speaker *spk = new (std::nothrow)speaker(kMixedUid, sample_rates_,
        stored_fmt_, slice_, stored);
...
bool audio_event_handler::onPlaybackFrameUid(unsigned uid, void *audioSample,
  ...
  if (it == speakers_.end()) {
    ...
    char location[512];
    if (is_batch_mode_) {
      snprintf(location, 511, "%s%s-%u", path_prefix_.c_str(),
          channel_name_.c_str(), uid);
      stored = location;
    }

    speaker *spk = new (std::nothrow)speaker(uid, sample_rates_,
        stored_fmt_, slice_, stored);
...
bool audio_event_handler::send_user_join(uid_t uid, int elapsed) {
  if (!writer_)
    return true;

  p_user_joined join;
  join.uid = uid;
  join.elapsed = elapsed;
  if (writer_->write_packet(join, 10) == base::kWriteOk)
    return true;
  return false;
}

bool audio_event_handler::send_user_offline(uid_t uid, int reason) {
  if (!writer_)
    return true;
  ...
  if (writer_->write_packet(offline, 10) == base::kWriteOk)
    return true;
  return false;
}
